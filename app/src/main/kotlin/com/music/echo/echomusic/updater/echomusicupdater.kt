...
package com.snuggle.music.snugglemusic.updater
...
import com.snuggle.music.snugglemusic.updater.downloadmanager.UpdateDownloadWorker
import com.snuggle.music.snugglemusic.updater.downloadmanager.DownloadNotificationManager
...
sealed class SnuggleUpdateStatus {
    object Idle : SnuggleUpdateStatus()
    object Checking : SnuggleUpdateStatus()
    ...
    data class NoUpdate(val version: String) : SnuggleUpdateStatus()
    data class Error(val message: String) : SnuggleUpdateStatus()
}
...
fun UpdateScreen(navController: NavHostController) {
    ...
    var status by remember { mutableStateOf<SnuggleUpdateStatus>(SnuggleUpdateStatus.NoUpdate(BuildConfig.VERSION_NAME)) }
    ...
    fun triggerUpdateCheck() {
        status = SnuggleUpdateStatus.Checking
        scope.launch {
            ...
            checkForUpdate(
                ...
                onSuccess = { tag, isAvailable, changelog, size, date, description, imageUrl, apkUrl ->
                    ...
                    status = if (isAvailable) {
                        ...
                        SnuggleUpdateStatus.NoUpdate(tag)
                    }
                },
                onError = {
                    status = SnuggleUpdateStatus.Error(context.getString(R.string.cant_check_updates))
                }
            )
        }
    }
    ...
    Scaffold(
        topBar = {
            LargeTopAppBar(
                title = {
                    val titleText = if (status is SnuggleUpdateStatus.Available) {
                        buildAnnotatedString {
                            ...
                            withStyle(
                                ...
                                append((status as SnuggleUpdateStatus.Available).version)
                            }
                        }
                    } else {
            ...
            Surface(
                ...
                Column(
                    ...
                    when (val currentStatus = status) {
                        is SnuggleUpdateStatus.Idle, is SnuggleUpdateStatus.Checking, is SnuggleUpdateStatus.NoUpdate, is SnuggleUpdateStatus.Error -> {
                            AnimatedActionButton(
                                text = stringResource(R.string.check_for_update),
                                onClick = { triggerUpdateCheck() },
                                enabled = currentStatus !is SnuggleUpdateStatus.Checking && !isDownloading,
                                modifier = Modifier.fillMaxWidth()
                            )
                        }

                        is SnuggleUpdateStatus.Available -> {
                            Row(
                                modifier = Modifier.fillMaxWidth(),
                                ...
                                AnimatedActionButton(
                                    ...
                                    onClick = {
                                        if (isDownloadComplete) {
                                            ...
                                            val urlToDownload = currentStatus.apkUrl ?: "https://github.com/prasanna172605/Snugle-Musix/releases/download/${currentStatus.version}/snugglemusix.apk"
        ...
        Box(
            ...
            LazyColumn(
                ...
                item {
                    val contentModifier = if (status is SnuggleUpdateStatus.Available) {
                    ...
                    Box(
                        ...
                        when (val currentStatus = status) {
                            is SnuggleUpdateStatus.Checking -> {
                                androidx.compose.material3.ContainedLoadingIndicator(
                                    modifier = Modifier.size(64.dp)
                                )
                            }

                            is SnuggleUpdateStatus.NoUpdate -> {
                                Column(
                                    horizontalAlignment = Alignment.CenterHorizontally
                                ) {
                                    Icon(
