            ...
            try {
                saavnAttempt = kotlinx.coroutines.withTimeoutOrNull(15000L) {
                    ...
                    val scored = songs.map { candidate ->
                        ...
                        val finalScore = titleScore + durationScore + artistScore + explicitScore + variantPenalty
                        
                        Timber.tag(TAG).d("Saavn Candidate: title=\"${candidate.name}\", artist=\"$saavnArtists\", duration=${saavnDuration}s | Sub-scores: title=$titleScore, artist=$artistScore, duration=$durationScore, explicit=$explicitScore, variant_penalty=$variantPenalty | Final Score: $finalScore")
